    ...
    if (favsArray.length > 0 && Object.keys(favsArray[0].data).length > 0) {
      return (
        <div className={styles.favorites}>
          {favsArray.map((location) => (
            <WeatherCard key={location.data.id} weather={location.data} />
          ))}
        </div>
      );
    } else if (isLoading) {
      return (
        <img
          src="https://imoshep-weather-app.s3.eu-central-1.amazonaws.com/loading.gif"
          width="40"
          alt="loading favorite locations"
          className={styles.preLoader}></img>
      );
    } else return null;
  }
}
